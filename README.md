...
![Tool Interface](figures/MELAB.png)
...
- **MATLAB**: Tested on MATLAB version 2019a.
- **Brainstorm**: Tested on Brainstorm version 11-Dec-2024.
...
- **ROIs**: Bilateral ROIs are grouped into four functional categories of Angular, Frontal, Temporal, Lateral. See, `convertHCPScout` and `defineROIs_HCP` sub-functions for more details.
...
- **ROIs**: DK-based ROIs are grouped into categories of AngSmg (angular gyrus and supramarginal G.), Front (frontal), LatFront, LatTemp (lateral temporal), PeriSyl (perisylvian cortex), Tanaka ([Tanaka et al. (2013), *AJNR Am J Neuroradiol.*](https://pubmed.ncbi.nlm.nih.gov/22878013/)), Temp (temporal), and Whole (all rois). See `defineROIs_DK` for the details.
...
3. **Run the Custom Process**:
    - In the Brainstorm **Process/Sources** tab, select **Compute LI (HCP or DK atlas, surface-based)**.
